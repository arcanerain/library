...
describe LibrariesController do
  ...
  def mock_library(stubs={})
    @mock_library ||= mock_model(Library, stubs).as_null_object
  end

  describe "GET index" do
    it "assigns all libraries as @libraries" do
      Library.stub(:all) { [mock_library] }
      get :index
      assigns(:libraries).should eq([mock_library])
    end
  end

  describe "GET show" do
    it "assigns the requested library as @library" do
      Library.stub(:find).with("37") { mock_library }
      get :show, :id => "37"
      assigns(:library).should be(mock_library)
    end
  end

  describe "GET new" do
    it "assigns a new library as @library" do
      Library.stub(:new) { mock_library }
      get :new
      assigns(:library).should be(mock_library)
    end
  end
  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created library as @library" do
        Library.stub(:new).with({'these' => 'params'}) { mock_library(:save => true) }
        post :create, :library => {'these' => 'params'}
        assigns(:library).should be(mock_library)
      end

      it "redirects to the created library" do
        Library.stub(:new) { mock_library(:save => true) }
        post :create, :library => {}
    ...
    describe "with invalid params" do
      it "assigns a newly created but unsaved library as @library" do
        Library.stub(:new).with({'these' => 'params'}) { mock_library(:save => false) }
        post :create, :library => {'these' => 'params'}
        assigns(:library).should be(mock_library)
      end

      it "re-renders the 'new' template" do
        Library.stub(:new) { mock_library(:save => false) }
        post :create, :library => {}
  ...
  describe "PUT update" do
    ...
    describe "with valid params" do
      it "updates the requested library" do
        Library.should_receive(:find).with("37") { mock_library }
        mock_library.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, :id => "37", :library => {'these' => 'params'}
      end

      it "assigns the requested library as @library" do
        Library.stub(:find) { mock_library(:update_attributes => true) }
        put :update, :id => "1"
        assigns(:library).should be(mock_library)
      end
    ...
    describe "with invalid params" do
      it "assigns the library as @library" do
        Library.stub(:find) { mock_library(:update_attributes => false) }
        put :update, :id => "1"
        assigns(:library).should be(mock_library)
      end
  ...
  describe "DELETE destroy" do
    ...
    it "redirects to the libraries list" do
      Library.stub(:find) { mock_library }
      delete :destroy, :id => "1"
